...
<head>
    ...
    <title>Internet_project (2)</title>
    ...
    <nav class="navbar navbar-light navbar-expand-md bg-light py-3" data-aos="fade-down" data-aos-duration="550" style="padding: 0px 0px;">
        <div class="container"><a class="navbar-brand d-flex align-items-center" href="index.php"><span class="bs-icon-sm bs-icon-rounded bs-icon-primary d-flex justify-content-center align-items-center me-2 bs-icon"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-book">
            ...
            <div class="collapse navbar-collapse" id="navcol-2">
                ...
                </ul><a class="nav-link" href="index.php"><button class="btn btn-primary" type="button">Log Out</button></a>
            </div>
        </div>
    </nav>
    <div>
        <div id="studentDetailsDiv" style='height:500px;'>
                        <?php
                            $email = $_POST["email"];
                            $password = $_POST["password"];
                            $conn = mysqli_connect("localhost", "root", "", "online_courses");
                            $stmt = "select * from `student` where `email` = '$email' and `password` = '$password' ";
                            $result = mysqli_query($conn, $stmt);
                            $login = false;
                            while ($row = mysqli_fetch_array($result) )
                            {
                            echo "<h1 style='text-align:center;'>Welcome $row[1]</h1><br>";
                            echo "<p style='color:white;'> ID: $row[0]</p> <p style='color:white;'>Name: $row[1]</p> <p style='color:white;'>Mobile Number: $row[2]</p> <p style='color:white;'>Email: $row[3]</p> <p style='color:white;'>Password: $row[4]</p>";
                            $login=true;
                            }
                            if($login==false)
                            echo '<script>alert("Wrong email/password"); window.open("login.php", "_self");</script>';
                            ?>
        </div>
    </div>
    <main>
